...
const DirectoresTecnicos = () => {
  ...
  const [vistaActual, setVistaActual] = useState('tipos');
  ...
  const eliminarTipo = async (id) => {
    if (window.confirm('¿Seguro querés eliminar este Tipo de Director? Se desvincularán los directores asociados.')) {
      ...
      if (error) {
        ...
        return;}
      setTiposDT(tiposDT.filter(t => t.id !== id));}};
  ...
  const guardarDT = async (e) => {
    ...
    const payload = {
      ...formDT,
      fecha_vencimiento: formDT.fecha_vencimiento || null,
      id_tipo: formDT.id_tipo || null};
  ...
  const renderListadoDirectores = () => {
    let dtsAMostrar = vistaActual === 'filtrados' 
      ? directores.filter(d => d.id_tipo === tipoSeleccionado?.id)
      : directores;
    if (filtroTexto) {
    ...
    if (filtroProfesion) {
      dtsAMostrar = dtsAMostrar.filter(d => d.profesion === filtroProfesion);
    }
    ...
    return (
      <div className="cifas-card">
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '20px', background: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', flexWrap: 'wrap' }}>
          ...
          {(filtroTexto || filtroProfesion) && (
            <button 
              onClick={() => { setFiltroTexto(''); setFiltroProfesion(''); }} 
        ...
        {dtsAMostrar.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', fontSize: '13px', color: '#64748b' }}>
            <span>
              Mostrando {indicePrimerItem + 1} a {Math.min(indiceUltimoItem, dtsAMostrar.length)} de {dtsAMostrar.length} directores
            </span>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button 
